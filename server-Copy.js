...
db.serialize(() => {
  ...
  const produtos = [
    ["0070847022015", "Monster Energy", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+1"], //ok
    ["7898938890076", "Monster Energy Zero Sugar", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+2"], //ok
    ["0070847033929", "Monster Ultra Violet", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+3"], //ok
    ["0070847022206", "Monster Ultra Zero", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+4"], //ok
    ["7898938890090", "Monster Ultra Peachy", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+5"], //ok
    //["7898938890090", "Monster Ultra Fiesta", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+6"],// <<<<<<<<<<<<<<<<<<
    ["7898938890045", "Monster Pipeline Punch", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+7"],//ok
    //["7898910847086", "Monster Ultra Paradise", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+8"], // <<<<<<<<<<<<<<<<<<
    //["7898910847093", "Monster Dragon Ice Tea Peach", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+9"],// <<<<<<<<<<<<<<<<<<
    ["0070847034803", "Monster Dragon Ice Tea Lemon", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+10"], //ok
    ["7898938890120", "Monster Rio Punch", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+11"], //ok
    ["0070847022305", "Monster Absolutely Zero", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+12"], //ok
    ["1220000250147", "Monster Khaotic", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+13"], //ok
    ["1220000250031", "Monster Pacific Punch", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+14"], //ok
    ["0070847033301", "Monster Mango Loco", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+15"], //ok
    ["1220000250222", "Monster Watermelon", "https://via.placeholder.com/300x600/1a1a1a/16a34a?text=Monster+16"] //ok
  ];
...
async function consultarPromocoes() {
  ...
  try {
    ...
    for (const produto of produtos) {
      try {
        const response = await fetch(`${API_CONFIG.baseURL}produto/pesquisa`, {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          console.error(`❌ Erro na API para ${produto.nome}: ${response.status}`);
          continue;
        }
...
1. Crie uma pasta para o projeto:
   mkdir monster-promo-backend
   cd monster-promo-backend

2. Inicialize o npm:
   npm init -y

3. Instale as dependências:
   npm install express sqlite3 node-cron cors dotenv

4. Crie o arquivo .env:
   APP_TOKEN=seu_token_aqui

5. Cole este código em server.js
...
No código, localize a seção "Popular produtos iniciais" (linha ~53)
Substitua:
- GTIN_MONSTER_X pelos GTINs reais
- Monster X pelos nomes reais
- URLs das imagens pelas hospedadas por você

---

⚙️ CONFIGURAÇÕES:

Ajuste no objeto API_CONFIG (linha ~69):
- precoAlvo: preço máximo para considerar promoção
- municipio: código IBGE (padrão: Maceió)
...
4. Adicione variável de ambiente APP_TOKEN
5. Deploy!
...
GET /api/produtos
- Lista todos os produtos com contagem de promoções

GET /api/produtos/:id/promocoes
- Lista locais com promoção de um produto específico
